      ...
      try {
        ...
        if (response.status === 200 && Array.isArray(response.data.data)) {
          ...
          const sanitizedReports = response.data.data.map(report => ({
            ...report,
            status: report.status || 'pending' // Fallback to 'pending' if status is undefined
          }));
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center mb-8">
          <button 
            onClick={() => navigate(-1)} 
            className="mr-4 p-2 rounded-full bg-white shadow-md hover:bg-gray-100"
          >
  ...
  {filteredReports.map(report => (
    <div 
      ...
      className={`border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer ${
        report.ProvinceAdmin ? 'bg-green-100' : 'bg-white'
      }`}
    >
      ...
      {report.ProvinceAdmin && (
        <p className="text-gray-600 text-sm">
          Assigned to: <span className="font-medium text-blue-600">{report.ProvinceAdmin}</span>
        </p>
      )}
    </div>
  ))}
